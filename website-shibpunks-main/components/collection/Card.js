  ...
  return (
    <div className="mb-8 mx-2 md:w-1/2 lg:w-1/4 px-4 py-4 bg-white rounded-xl">
      <div className="rounded justify-center">
        ...
        <p className="text-red-500 mb-2 mt-1 text-xl text-center font-heading">Reward Multiplier: {NFTInfo.attributes?.find(x => x.trait_type == 'Reward Multiplier')?.value}</p>
        <p className="text-red-600 mt-2 text-lg text-center font-heading">
          ...
          {NFTInfo.attributes?.reduce(
            (acc, attr, i) =>
              rejectAttr.has(attr.trait_type)
                ...
                : [
                    ...acc,
                    <p className="capitalize text-red-400 text-md text-center font-heading" key={`${NFTInfo}-${i}`}>
